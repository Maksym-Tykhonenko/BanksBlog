...
import {KeyboardAvoidingView,Modal,ScrollView,TextInput,StyleSheet,ImageBackground, View,FlatList, Text, SafeAreaView, TouchableOpacity, Image, Alert } from "react-native";
...
const EuropeBanksScreen = ({ navigation }) => {
    ...
    removeData = async () => {
        try {
            await AsyncStorage.removeItem('addEuropeBanks')
        } catch (e) {
            // remove error
        }
    ...
    const storeData = async () => {
        try {
            ...
            await AsyncStorage.setItem('addEuropeBanks', jsonAddBanks)
        } catch (e) {
            console.log(e)
        }
    };
    ...
    const getData = async () => {
        try {
            const jsonAddBanks = await AsyncStorage.getItem('addEuropeBanks');
        ...
        <View style={styles.conteiner}>
            ...
            >
                <View style={styles.subConteiner}>
                    <Text style={{ fontWeight: 'bold', fontSize: 30, marginBottom: 30, color: '#103db1' }}>Europe Banks: </Text>
                            ...
                            >
                                ...
                                <View>
                                    {!selectPhoto ? (<TouchableOpacity
                                        onPress={() => { ImagePicer() }}
                                        style={{ marginLeft: 50, marginTop: 0, marginBottom: 5, borderWidth: 1, borderColor: 'yellow', borderRadius: 25, width: 150, height: 40, alignItems: 'center', justifyContent: 'center' }}
